...
struct Args 
{
    ...
    unsigned int left_cut;       // left cut value 
    unsigned int right_cut;      // right cut value 
    unsigned int windows_size;   // windows size to check 
    unsigned int min_len;        // Filter sequence shorter than min_len
    unsigned int max_len;        // Filter sequence larger than max_len
    unsigned int tb;             //removes n bases from the begins of sequencing fragments (int default 0)
    unsigned int te;             //removes n bases from the ends of sequencing fragments (int default 0)
    unsigned int binSize;        //to compute statistic per base (int default 1)
};
...
/* P R O T O T Y P E S *******************************************************/
void process_options(int argc, char *argv[], Args * args, Names * names);
...
void statistic_mean (unsigned int *quality1_int, int binSize,int size, int *max_read_len, unsigned int *maxQ, unsigned int *minQ, int *total_reads, int *qual_position, int bouderLeft1, int bouderRight1);
