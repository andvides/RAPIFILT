...
void construct_fastq_trimmed(FILE *fp, char *name, char *bases, unsigned int *quality, unsigned int nbases, struct Args * args, struct Names * names, int *bounders)
{
    unsigned int j=0;
    uint8_t quality_char;
    unsigned int bouderLeft=nbases-1; //por si no encuentra ninguna		
    unsigned int bounderRight=nbases-1;

    unsigned int sum=0, k=0, next=1;
    ...
    unsigned int readLenght=(bounderRight+1)-bouderLeft;
