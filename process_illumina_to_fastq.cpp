...
void process_illumina_to_fastq(struct Args * args, struct Names * names)
{
    ...
    bool gzFiles = false;
    if (names->illuminaFile1.compare(names->illuminaFile1.size()-3,3,".gz") == 0) {
        if (names->illuminaFile2.compare(names->illuminaFile2.size()-3,3,".gz") != 0) {
            cout << "Error: both files must be .gz" << endl;
            return;
        }
        gzFiles = true;
    }

    filebuf infile1, infile2;
    ifstream gzFile1,gzFile2;
    ...
    if (gzFiles) {
        ...
        gzFile2.open(file2, std::ios_base::in | std::ios_base::binary);
    ...
    unsigned int bouderLeft2=0, bouderRight2=0, finalSize2=0, size2=0;
    ...
    unsigned int maxQR[maxReadLen]={0}, minQR[maxReadLen];   
    unsigned int maxQFf[maxReadLen]={0}, minQFf[maxReadLen];
    unsigned int maxQRf[maxReadLen]={0}, minQRf[maxReadLen];   

    int max_read_lenF=0, max_read_lenR=0, max_read_lenFf=0,max_read_lenRf=0;
    ...
    while (next)
    {
        ...
        switch ( state ) 
        {
            ...
            case s3:
                ...
                break;
            case s4:
                while(instream2.get(word2) && word2!='\n') //reverse bases
    ...
    for(int i=0;i<index;i++) {
        if (total_readsFf[i] > 0)
          fprintf(stat_fp1, "%d%s", qual_position_Ff[i]/total_readsFf[i]," ");//std::cout << *it;
        else
          fprintf(stat_fp1, "%d%s", qual_position_Ff[i]," ");//std::cout << *it;
    }
    fprintf(stat_fp1, "\n");//cout<<endl;
    ...
    for(int i=0;i<index;i++)
        if (total_readsRf[i] > 0)
            fprintf(stat_fp2, "%d%s", qual_position_Rf[i]/total_readsRf[i]," ");//std::cout << *it;
        else
            fprintf(stat_fp2, "%d%s", qual_position_Rf[i]," ");//std::cout << *it;
    fprintf(stat_fp2, "\n");//cout<<endl;
